...
Draw.loadPlugin(function (ui) {
    ...
    // Show some available resources
    window.resKeys = Object.keys(window).filter(key => /mx/.test(key)).filter(key => !(obj instanceof Function && obj.length > 0));

    window.res = resKeys.map(key => window[key]);
